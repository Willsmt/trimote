...
Destilação histórica das features do Trimote (001→007). **Fonte de verdade: o
`specs/NNN/plan.md` de cada feature** — estes resumos são a versão curada e condensada, não o
contrato completo. Este arquivo **NÃO é carregado em contexto** — é consulta sob demanda.

Arquivado de `CLAUDE.md` (bloco SPECKIT) na migração para a arquitetura de skills (#45): o bloco
gerenciado pelo speckit-agent-context-update é sobrescrito a cada atualização e não pode guardar
conteúdo curado à mão. Ordem: mais recente primeiro.
